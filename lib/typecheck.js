...
function check(json, data) {
  ...
  keys.forEach(key => {
    ...
    if (typeof json[key] !== "object") {
      ...
      if (json[key].type) expect(data[key]).to.be.a(json[key].type);
      if (json[key].value) expect(data[key]).to.equal(json[key].value);
      if (json[key].children) check(json[key].children, data[key]);
    }
  });
}
